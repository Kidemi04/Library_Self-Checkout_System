		...
		<div className="rounded-2xl border border-swin-charcoal/10 bg-white shadow-sm shadow-swin-charcoal/5">
			...
			<div className="divide-y divide-swin-charcoal/10 bg-white">
				{items.length === 0 ? (
					...
					items.map((b) => (
						<div key={b.id} className="flex gap-4 px-6 py-4 items-start hover:bg-swin-ivory transition">
							<img
								src={b.cover ?? '/a.txt'}
								alt={`Cover of ${b.title}`}
								className="h-40 w-28 flex-none rounded object-cover border border-gray-100"
							/>
							...
							<div className="flex-1">
								<div className="flex items-start justify-between">
									...
									<div className="text-sm text-swin-charcoal/60">
										{b.available ? (
											<span className="inline-flex items-center rounded-full bg-swin-charcoal/10 px-3 py-1 text-xs font-semibold text-swin-charcoal">Available</span>
										) : (
											<span className="inline-flex items-center rounded-full bg-swin-red/10 px-3 py-1 text-xs font-semibold text-swin-red">Checked out</span>
										)}
									</div>
								</div>

												<p className="mt-2 text-sm text-swin-charcoal/60">{b.description ?? 'No description available.'}</p>
												...
												<div className="mt-3 flex flex-wrap gap-2">
													{(b.tags || []).map((t) => (
														<span key={t} className="text-[11px] bg-gray-100 px-2 py-1 rounded">{t}</span>
													))}
												</div>
							</div>
						</div>
