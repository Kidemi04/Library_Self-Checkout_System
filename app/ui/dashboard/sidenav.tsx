...
export default function SideNav({ user, isBypassed }: SideNavProps) {
  ...
  return (
    <aside
      className={clsx(
        'fixed left-0 top-0 flex h-screen w-64 flex-col overflow-y-auto scrollbar-none px-3 py-5 shadow-xl transition-colors duration-300 md:px-4',
        isDark ? 'bg-slate-950/95 text-slate-100 ring-1 ring-white/10' : 'bg-[#343642] text-[#FEFDFD] ring-1 ring-black/30',
      )}
    >
      <Link
      ...
      <div className="flex grow flex-col justify-between gap-6">
        <nav className="flex flex-col gap-2">
          {isPrivileged ? (
            <div
              className={clsx(
                'rounded-xl border p-4 text-sm shadow-inner transition-colors duration-300',
                isDark ? 'border-white/10 bg-white/5 text-slate-100' : 'border-[#FEFDFD]/15 bg-[#2a2d38] text-[#FEFDFD]',
              )}
            >
              <p
                className={clsx(
                  'text-[11px] uppercase tracking-wide',
                  isDark ? 'text-white/60' : 'text-[#FEFDFD]/80',
                )}
              >{`${roleLabel} access`}</p>
              <p className="mt-1 truncate text-sm font-semibold">{user.name ?? user.email ?? 'Librarian'}</p>
              {user.email ? (
                <p className={clsx('break-words text-[11px]', isDark ? 'text-white/60' : 'text-[#FEFDFD]/70')}>
                  {user.email}
                </p>
              ) : null}
              <p
                className={clsx(
                  ...
                  isDark ? 'text-emerald-300/80' : 'text-[#C82333]',
                )}
              >
          ...
          <SignOutButton
            className={clsx(
              ...
              isDark
                ...
                : 'border-[#FEFDFD]/30 bg-transparent text-[#FEFDFD] hover:bg-white/10 hover:text-[#FEFDFD]',
            )}
