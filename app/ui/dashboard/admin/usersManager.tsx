...
const mapRows = (rows: ManagedUserRow[]): ManagedUser[] =>
  rows.map((row) => {
...
export default function UsersManager({ initialUsers }: { initialUsers: ManagedUserRow[] }) {
  const router = useRouter();
  const [users, setUsers] = useState<ManagedUser[]>(() => mapRows(initialUsers));
  ...
  // Re-sync local state when the server-provided initialUsers changes
  // (e.g. after router.refresh() following an add/update/delete).
  useEffect(() => {
    setUsers(mapRows(initialUsers));
  }, [initialUsers]);
