...
export default async function UserDashboardPage() {
  ...
  return (
    <main className="space-y-8">
      ...
      <header className="grid gap-6 rounded-2xl border border-swin-ivory/10 bg-swin-charcoal p-8 text-swin-ivory shadow-inner shadow-black/20 md:grid-cols-[1fr_minmax(0,260px)] md:items-center">
        <div>
          <p className="text-sm uppercase tracking-wide text-swin-ivory/60">Self-Service Desk</p>
          <h1 className="mt-2 text-2xl font-semibold text-swin-ivory">
        ...
        <div className="rounded-2xl border border-swin-ivory/15 bg-swin-ivory/10 p-5 text-sm text-swin-ivory/90 shadow-inner shadow-black/10 space-y-2">
          <p className="text-xs uppercase tracking-wide text-swin-ivory/60">Signed in</p>
          {user.email ? <p className="text-base font-semibold text-swin-ivory">{user.email}</p> : null}
          <p className="inline-flex items-center gap-2 rounded-full border border-swin-ivory/30 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-swin-ivory/80">
            Role: Student
          </p>
          {isBypassed ? (
            <p className="rounded-md bg-amber-400/20 px-3 py-2 text-[11px] font-medium text-amber-200 shadow-inner shadow-amber-500/10">
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-lg font-semibold text-swin-ivory">Active loans</h2>
          <p className="text-sm text-swin-ivory/60">
